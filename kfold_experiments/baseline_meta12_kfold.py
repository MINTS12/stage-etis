"""
Binary Relevance baseline on the 12 metacategory labels -- k-fold version.

Sibling script to baseline_fine138_kfold.py, same design, targeting the
12 meta_* columns in hmcn_dataset.csv instead of the 138 fine_* columns.
Shares the exact same cached fold split (kfold_results/hmcn_kfold_split_indices.pkl)
with both the HMCN k-fold sweep and the fine-138 baseline, so all three are
evaluated on identical molecules per fold.

Differences from baseline_fine138_kfold.py:
    1. Targets meta_* (12 labels) instead of fine_* (138 labels).
    2. Outputs the meta-12 block of hmcn_eval's schema (roc_auc_12, pr_auc_12,
       f1_macro_12, instance_f1_12, balanced_accuracy_12, matched_accuracy_12,
       sensitivity_macro_12, specificity_macro_12, precision_macro_12,
       recall_macro_12, hamming_loss_12, jaccard_12, hier_violation_rate_12
       (hardcoded 0.0 -- nothing sits above the meta level, same as HMCN's own
       convention), label_cooc_consistency_12). All *_138 columns and test_loss
       are left NaN via hmcn_eval.save_experiment's automatic fill, exactly as
       the fine-138 script leaves the meta/hierarchy columns NaN.
    3. Formulas for specificity_macro_12 and label_cooc_consistency_12 are not
       reimplemented -- called directly from hmcn_eval (hmcn_eval._macro_specificity,
       hmcn_eval._label_cooccurrence_consistency) to avoid any drift from the
       HMCN script's exact definitions. Everything else is a direct sklearn call,
       copied from hmcn_eval.compute_all_metrics' meta-12 block.
    4. Same fixed-grid-per-fold structure as fine-138 (post-fix): every
       (model, config) in MODEL_GRIDS runs identically on every fold, no
       per-fold GridSearchCV auto-tuning.

Usage:
    Smoke test:  DEBUG=True in CONFIG, then run (uses 1 fold, 1 model, tiny grid).
    Full run (server, background):
        nohup python3 -u baseline_meta12_kfold.py > /dev/null 2>&1 &
...
OUTPUT_DIR   = Path("outputs_meta12_kfold")
LOG_DIR      = Path("logs")
RANDOM_STATE = 42
K            = 5
...
if DEBUG:
    OUTPUT_DIR = Path("outputs_meta12_kfold_debug")
    MODELS_TO_RUN = ["LR"]
...
log_path = LOG_DIR / f"baseline_meta12_kfold_{'debug' if DEBUG else 'full'}_{timestamp}.log"

logger = logging.getLogger("baseline_meta12_kfold")
...
def compute_meta_only_metrics(meta_probs, meta_true, meta_thresholds, Y_train):
    ...
    and label_cooc_consistency_12 are non-trivial (confusion-matrix-based /
    co-occurrence-based), so they're called directly from hmcn_eval rather than
    reimplemented, to guarantee exact fidelity with the HMCN script's numbers.
    hier_violation_rate_12 is hardcoded 0.0, matching hmcn_eval's own convention
    (nothing sits above the meta level -- it's a sanity-check constant, not
    something that varies by model). All *_138 columns and test_loss are left
    NaN via hmcn_eval.save_experiment's automatic fill.
    """
...
def evaluate(name, param_value, fold_i, clf, X_train, y_train, X_val, y_val, X_test, y_test, label_cols):
    ...
    # Same 19-candidate sweep in [0.05, 0.95] as HMCN's calibration.
...
def run_model(name, cfg, param_value, fold_i, X_train, y_train, X_val, y_val, X_test, y_test,
    ...
    per_label_df, ablation_metrics = evaluate(name, param_value, fold_i, clf, X_train, y_train, X_val, y_val, X_test, y_test, label_cols)

    config = dict(
        experiment='baseline_meta12_kfold', param_name='model', param_value=param_value,
...
def main():
    ...
    results_path = OUTPUT_DIR / "baseline_meta12_kfold_all_results.csv"
    ablation_csv_path = OUTPUT_DIR / "baseline_meta12_kfold_ablation_results.csv"
    ...
    # Per-fold macro average (mean across the 12 labels), one row per (fold, Model, Config)
    per_fold_macro = all_results_df.groupby(["Fold", "Model", "Config"])[metric_cols].mean().round(3)
    per_fold_macro.to_csv(OUTPUT_DIR / "baseline_meta12_kfold_per_fold_macro.csv")
    ...
    summary.to_csv(OUTPUT_DIR / "baseline_meta12_kfold_summary_mean_std.csv")
    ...
    if ablation_csv_path.exists():
        ...
        ablation_summary_path = OUTPUT_DIR / "baseline_meta12_kfold_ablation_summary_mean_std.csv"
        ...
        # One row per model: the single best config (by mean pr_auc_12 across folds) --
        ...
        winners_path = OUTPUT_DIR / "baseline_meta12_kfold_winning_configs.csv"
        winners.to_csv(winners_path)
        logger.info("Winning config per model (best mean pr_auc_12 across folds):\n" +
                    winners[['pr_auc_12', 'f1_macro_12', 'roc_auc_12']].to_string())
