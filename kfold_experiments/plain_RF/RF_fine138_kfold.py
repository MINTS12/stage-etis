...
Native multi-output RandomForest baseline (138 fine-grained odor labels),
run through the same k=5 IterativeStratification fold splits used by
baseline_fine138_kfold.py (BR+RF) and the HMCN-F kfold pipeline, so this
number is directly comparable to both.

Unlike BR+RF (skmultilearn.problem_transform.BinaryRelevance, which trains
138 fully independent RandomForestClassifiers), this fits ONE
RandomForestClassifier per fold on the full (n_samples, 138) label matrix
directly -- sklearn's tree ensembles support multi-output natively, so
splits are chosen jointly across all 138 labels rather than in isolation.
...
Usage (on espadon1):
    nohup python3 -u RF_fine138_kfold.py > rf_fine138_stdout.log 2>&1 &
...
LOG_PATH     = Path("RF_fine138_kfold.log")

LABEL_PREFIX = "fine_"     # 138 fine-grained descriptors
N_FOLDS = 5
...
def main():
    ...
    per_run_path = OUTPUT_DIR / "RF_fine138_kfold_per_run.csv"
    ...
    summary_path = OUTPUT_DIR / "RF_fine138_kfold_summary.csv"
