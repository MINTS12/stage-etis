"""
Plain flat MLP baseline -- two hidden layers, one output layer, single BCE
loss. No local/global split, no beta blend, no hierarchy violation loss.
Jointly predicts fine138 + meta12 as one concatenated 150-dim sigmoid output,
using the shared 5-fold split cache used by every other k-fold script in
this project.

Purpose: the previous mlp_kfold.py (JointMLP) still shared HMCN-F's
local/global/beta-blend skeleton, just without the violation loss term --
useful for isolating that one term, but not a "does going neural help at
all" baseline in its own right. This script is that baseline: the simplest
possible neural model on the same features, same fixed-config-per-fold
discipline, same split cache.

    Input -> Linear -> BatchNorm -> ReLU -> Dropout   (hidden 1)
          -> Linear -> BatchNorm -> ReLU -> Dropout   (hidden 2)
          -> Linear(150) -> Sigmoid                    (output)
    Loss = BCE(P, concat(Y_fine138, Y_meta12))

The fine/meta split only exists at evaluation time (P[:, :138] / P[:, 138:]).

Design notes / things to verify before treating these numbers as final:
    - meta12 metric key names (roc_auc_12, pr_auc_12, etc.) are named by
      direct analogy with this project's existing _138 convention -- cross-
      check against hmcn_eval.py's own naming before merging result tables.
    - hier_violation_rate_138 is computed explicitly here (same formula as
      mlp_kfold.py: fraction of samples where a fine label is predicted
      positive but its meta parent isn't, averaged over pairs) purely as a
      post-hoc diagnostic -- this model was never penalized for violating
      it, so a high rate here is expected and informative, not a bug.
    - Early stopping / model selection uses val macro PR-AUC (mean of
      fine138 PR-AUC and meta12 PR-AUC), matching mlp_kfold.py.
      'val_pr_auc_combined' is inserted into hmcn_eval._CSV_COLUMNS the
      same guarded way as in mlp_kfold.py -- if that script already ran
      against the same hmcn_eval module instance this is a no-op.

Usage:
    Smoke test:  DEBUG=True in CONFIG, then run (1 fold, 2 configs, few epochs).
    Full run (server, background):
        nohup python3 -u plain_mlp_kfold.py > /dev/null 2>&1 &
...
# Same guarded-insert pattern as baseline_fine138_kfold.py / mlp_kfold.py.
...
OUTPUT_DIR   = Path("outputs_plain_mlp_kfold")
LOG_DIR      = Path("logs")
RANDOM_STATE = 42
K            = 5
...
if DEBUG:
    OUTPUT_DIR = Path("outputs_plain_mlp_kfold_debug")
    GRID = {"hidden1_dim": [128], "hidden2_dim": [64], "dropout": [0.3]}
    EPOCHS = 5
    PATIENCE = 3
    DEBUG_N_FOLDS = 1
...
log_path = LOG_DIR / f"plain_mlp_kfold_{'debug' if DEBUG else 'full'}_{timestamp}.log"

logger = logging.getLogger("plain_mlp_kfold")
...
# identical to baseline_fine138_kfold.py / mlp_kfold.py.
# ======================================================================
...
# ======================================================================
# MODEL -- plain flat MLP: two hidden layers, one output layer, no
# branching, no blending. See module docstring for the exact layout.
# ======================================================================
class PlainMLP(nn.Module):
    def __init__(self, idim, n_out, h1, h2, dropout):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(idim, h1), nn.BatchNorm1d(h1), nn.ReLU(), nn.Dropout(dropout),
...
@torch.no_grad()
def collect(model, loader, device, n_fine):
    """Returns fine_probs, fine_true, meta_probs, meta_true, sliced from the
    single concatenated 150-dim output at n_fine."""
    model.eval()
    fp, ft, mp, mt = [], [], [], []
    for Xb, Y1b, Y2b in loader:
        P = model(Xb.to(device)).cpu().numpy()
        fp.append(P[:, :n_fine]); mp.append(P[:, n_fine:])
        ft.append(Y1b.numpy()); mt.append(Y2b.numpy())
...
# ======================================================================
# METRICS (mirrors mlp_kfold.py's compute_metrics)
# ======================================================================
...
def per_label_rows(fold_i, param_value, label_set, label_names, probs, true, pred, thresholds):
    ...
    for i, label in enumerate(label_names):
        ...
        rows.append({
            "Fold": fold_i, "Model": "PlainMLP", "Config": param_value,
...
def run_config(cfg, param_value, fold_i, fold_data, fine_names, meta_names, pairs,
    ...
    n_fine = y1_tr.shape[1]
    n_out = n_fine + y2_tr.shape[1]
    ...
    model = PlainMLP(
        idim=X_train.shape[1], n_out=n_out,
        h1=cfg['hidden1_dim'], h2=cfg['hidden2_dim'], dropout=cfg['dropout'],
    ).to(device)
    optimizer = torch.optim.Adam(model.parameters(), lr=LR, weight_decay=WEIGHT_DECAY)
    scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(optimizer, patience=8, factor=0.5)

    logger.info(f"  [PlainMLP] fold {fold_i}: fitting {param_value}")
    t0 = time.time()
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        for Xb, Y1b, Y2b in train_loader:
            Xb = Xb.to(device)
            Yb = torch.cat([Y1b, Y2b], 1).to(device)
            optimizer.zero_grad()
            P = model(Xb)
            loss = bce(P, Yb)
            loss.backward()
        ...
        fp_v, ft_v, mp_v, mt_v = collect(model, val_loader, device, n_fine)
    ...
    model.to(device)
    logger.info(f"  [PlainMLP] fold {fold_i} [{param_value}]: fit done in {time.time() - t0:.1f}s, "
    ...
    fp_t, ft_t, mp_t, mt_t = collect(model, test_loader, device, n_fine)
    ...
    logger.info(f"  [PlainMLP] fold {fold_i} [{param_value}] TEST  "
    ...
    config = dict(
        experiment='plain_mlp_kfold', param_name='model', param_value=param_value,
        fold=fold_i, model='PlainMLP', seed=RANDOM_STATE,
        # global_dim/local_dim/lambda_viol/beta: not applicable to this architecture --
        # left unset, hmcn_eval.save_experiment NaN-fills them (same convention as
        # baseline_fine138_kfold.py's sklearn models).
        hidden1_dim=cfg['hidden1_dim'], hidden2_dim=cfg['hidden2_dim'], dropout=cfg['dropout'],
...
def format_param_value(cfg):
    return "PlainMLP_" + "_".join(f"{k}={v}" for k, v in cfg.items())
...
def main():
    ...
    results_path = OUTPUT_DIR / "plain_mlp_kfold_all_results.csv"
    ablation_csv_path = OUTPUT_DIR / "plain_mlp_kfold_ablation_results.csv"
    ...
    per_fold_macro.to_csv(OUTPUT_DIR / "plain_mlp_kfold_per_fold_macro.csv")
    ...
    summary.to_csv(OUTPUT_DIR / "plain_mlp_kfold_summary_mean_std.csv")
    ...
    if ablation_csv_path.exists():
        ...
        ablation_summary_path = OUTPUT_DIR / "plain_mlp_kfold_ablation_summary_mean_std.csv"
        ablation_summary.to_csv(ablation_summary_path)
        logger.info(f"Ablation-format mean +/- std summary (compare directly against "
                    f"HMCN-F and JointMLP's own summary CSVs): {ablation_summary_path}")
        ...
        winners_path = OUTPUT_DIR / "plain_mlp_kfold_winning_config.csv"
