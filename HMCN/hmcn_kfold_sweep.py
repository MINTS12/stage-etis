...
def find_thr(preds, targets):
    ...
    thr = np.full(targets.shape[1], 0.5)
...
def mf1(p, t, thr):
    return f1_score(t, (p >= thr[np.newaxis, :]).astype(int), average='macro', zero_division=0)


# ============================== TRIAL ==============================================

def trial(cfg, data, pairs, device, epochs=EPOCHS, patience=PATIENCE, seed=SEED):
    ...
    fp_v, ft_v, mp_v, mt_v = collect(m, val, device)
    fthr = find_thr(fp_v, ft_v)
    mthr = find_thr(mp_v, mt_v)
    fp_t, ft_t, mp_t, mt_t = collect(m, tel, device)

    return {
        'val_auc': best_auc, 'best_epoch': ep,
        'meta_auc': mauc(mp_t, mt_t), 'meta_f1': mf1(mp_t, mt_t, mthr),
        'fine_auc': mauc(fp_t, ft_t), 'fine_f1': mf1(fp_t, ft_t, fthr),
    }
...
def run_kfold_sweep(csv_path=CSV_PATH, k=K, val_ratio=VAL_RATIO, grid=GRID,
    ...
    all_rows_path = os.path.join(results_dir, 'hmcn_kfold_all_results.csv')
    fieldnames = ['fold'] + keys + ['val_auc', 'best_epoch', 'meta_auc', 'meta_f1', 'fine_auc', 'fine_f1']
    ...
    if os.path.exists(all_rows_path):
        ...
        for _, row in prev.iterrows():
            completed.add((int(row['fold']),) + tuple(round(float(row[k]), 8) for k in keys))
        print(f"Found {len(completed)} completed runs in '{all_rows_path}' — these will be skipped.")

    write_header = not os.path.exists(all_rows_path)
    csv_file = open(all_rows_path, 'a', newline='')
    ...
    for fold_i, (train_idx, val_idx, test_idx) in enumerate(fold_splits):
        ...
        # Only build (and scale) this fold's data if there's at least one config
        # left to run on it — skips fold entirely if it was fully completed before.
        ...
        for i, cfg in enumerate(configs, 1):
            if cfg_key(fold_i, cfg) in completed:
                ...
                continue
            r = trial(cfg, data, pairs, device, epochs=epochs, patience=patience)
            row = {'fold': fold_i, **cfg, **r}
            writer.writerow(row); csv_file.flush()
            print(f"  [{i}/{len(configs)}] {cfg} -> meta_auc={r['meta_auc']:.4f} "
                  f"meta_f1={r['meta_f1']:.4f} fine_auc={r['fine_auc']:.4f} fine_f1={r['fine_f1']:.4f}")

    csv_file.close()

    combined = pd.read_csv(all_rows_path)
    metric_cols = ['val_auc', 'meta_auc', 'meta_f1', 'fine_auc', 'fine_f1']
    summary = combined.groupby(keys)[metric_cols].agg(['mean', 'std'])
    summary_path = os.path.join(results_dir, 'hmcn_kfold_summary_mean_std.csv')
    summary.to_csv(summary_path)

    print(f"\n{'='*60}\nK-fold sweep complete.")
    print(f"  Per-run results: '{all_rows_path}'")
